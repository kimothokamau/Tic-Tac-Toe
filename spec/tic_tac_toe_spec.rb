...
describe WinCombinations do
  describe '#win_combos' do
    it 'compares if player x is the winner' do
      ...
      expect(p(win_combinations.win_combos([1, 2, 3], [1, 2, 3, 4, 5, 6, 7], "Player X"))).to eql("Congratulations Player X! You are the winner!")
  end

  it 'compares if player y is the winner' do
    win_combinations = WinCombinations.new
    expect(p(win_combinations.win_combos([3, 6, 9], [1, 2, 3, 4, 5, 6, 7, 8], "Player Y"))).to eql("Congratulations Player Y! You are the winner!")
    end

  it 'compares if nobody won and the game was drawn' do
    win_combinations = WinCombinations.new
    expect(p(win_combinations.win_combos([1, 2, 5], [1, 2, 3, 4, 5, 6, 7, 8, 9], "Player X"))).to eql("It's a draw. Better luck next time guys.")
    end
end
end
